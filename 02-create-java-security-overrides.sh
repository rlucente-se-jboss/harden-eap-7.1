...
sed -i.bak "s/\(com\.sun\.net\.ssl\.internal\.ssl\.Provider\)$/\1 SunPKCS11-nss-fips/g" "$tmpfile"

echo "security.provider.1=sun.security.pkcs11.SunPKCS11 \${user.home}/nss-pkcs11-fips.cfg" >> $HOME/java.security.properties
cat "$tmpfile" >> $HOME/java.security.properties

cat >> $HOME/java.security.properties <<END2
...
cat > $HOME/nss-pkcs11-fips.cfg <<END3
name = nss-fips
